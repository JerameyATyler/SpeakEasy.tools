...
import {GetVocabulary, UpdateVocabularyList} from "../../Queries";
...
export default ({vocabId, vocabName, vocabDescription, translations}) => {
                ...
                <div className={clsx(classes.row)}>
                    {loading ? (
                        ...
                        <Autocomplete
                            ...
                            onChange={(e, v) => setNewTranslations([...v])}
                        />
                    )}
                </div>
            ),
    ...
    const [insertTranslations, setInsertTranslations] = useState(null);

    const [vocabulary, loading] = GetVocabulary(searchWord);
    const [formattedVocabulary, setFormattedVocabulary] = useState([]);

    const updatedRows = UpdateVocabularyList(vocabId, vocabName, vocabDescription, insertTranslations);
    ...
    const handleAdd = () => {
        setInsertTranslations(JSON.stringify([...translations, {word: newWord, translations: [...newTranslations]}]));
    };
